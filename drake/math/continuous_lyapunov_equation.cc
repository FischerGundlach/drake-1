...
Matrix2d Solve2By2ContinuousLyapunovEquation(
  ...
  std::cout << "Vector A_vec: " << A_vec << std::endl;
  std::cout << "Vector Q_vec: " << Q_vec << std::endl;
...
MatrixXd SolveReducedContinuousLyapunovEquation(
  ...
  if (m == 1) {
    ...
    int n{1};  // n equals the size of the n-by-n block in the left-upper
               // corner of S.
    std::cout << "(!(is_approx_equal_abstol(S.block(1, 0, 1, 1), Vector1d(0), "
                 "1e-3))): "
              << (!(is_approx_equal_abstol(S.block(1, 0, 1, 1), Vector1d(0),
                                           1e-3)))
              << std::endl;
    std::cout << "S.block(1, 0, 1, 1): " << S.block(1, 0, 1, 1) << std::endl;
    std::cout << "Vector1d(0): " << Vector1d(0) << std::endl;
    ...
    if (n == 1) {
      ...
      std::cout << "Before assigning to S_1_vec." << std::endl;
      std::cout << "S_1_vec cols, rows" << S_1_vec.rows() << ","
                << S_1_vec.cols() << std::endl;
      std::cout << "S_1 cols, rows" << S_1.rows() << "," << S_1.cols()
                << std::endl;
      S_1_vec << S_1.transpose(), MatrixXd::Zero(m - 2, m - 2),
          MatrixXd::Zero(m - 2, m - 2), S_1.transpose();
      MatrixXd s_11_vec(2 * (m - 2), 2 * (m - 2));
      std::cout << "Before assigning to s_11_vec." << std::endl;
      ...
      std::cout << "Before assigning to rhs." << std::endl;
      ...
      x_bar.row(0) = x_bar_vec.segment(0, m - 2);
      x_bar.row(1) = x_bar_vec.segment(m - 2, m - 2);
    }
    std::cout << "After determined part." << std::endl;
    ...
    std::cout << "Before recursion call." << std::endl;
